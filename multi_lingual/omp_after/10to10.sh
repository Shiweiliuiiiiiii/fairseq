...
pretrained_model=finetuned_models/10to10/checkpoint_last_iter0.pt
save_dir=omp_after_10to10
sparsity=$2
state=$3
...
CUDA_VISIBLE_DEVICES=$1 python train.py "$path_2_data" \
    ...
    --seed 222 --sparse --log-format simple --log-interval 100 --save-dir $save_dir/$state/ --fix --sparse-init one_shot_gm_cpu --sparsity $sparsity

# Calculate Scores
bash multi_lingual/omp_after/score.sh $save_dir/$state/ ${save_dir}_$state $path_2_data $lang_list $lang_pairs $1
